=begin
This one has been out there for a little while, it is a common problem with writing reports, etc.

What the challenge is write a program that provides the following output, with
the following sample of use. The output is shown first, I will make some notes
about it to pay attention to below:
...
# What is the maximum size of the product of n^2
def spacing(number)
  (number ** 2).to_s.length.next
end

def decore(number, decorate)
  decorate * spacing(number) * number
end
def multiplication_table(number, heading = '', decorate = '')
  result = ''
  result << heading << "\n" unless heading.empty?
  result << decore(number, decorate) << "\n" unless decorate.empty?
  result << table(number)
  result << decore(number, decorate) << "\n" unless decorate.empty?
  result
end
def table(number)
  (1..number).map do |x|
    table ||= ''
    (1..number).map do |y|
      table << (("%#{spacing(number)}d" % (x * y)).lstrip).rjust(spacing(number))
    end
    table << "\n"
  end.join
end

table1 = multiplication_table 9, 'Times table to 9','='
table2 = multiplication_table 20
...
if __FILE__ == $PROGRAM_NAME
  ...
  describe 'multiplication tables' do
    it 'must have a nine by table for multiplication_table(9)' do
      multiplication_table(9).must_equal DATA.read
    end
  end
end
