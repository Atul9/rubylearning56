...
from the timestamp recorded inside the file, the game refuses to load the save
file.
...
The load_game method is responsible for comparing the timestamp within the file
to the time the filesystem has associated with the file. Write the
...
command, you shouldn't depend on these methods to defend you against a skilled
attacker actively trying to fool your program. 

Read up on sleep method and Time class.
=end
def save_game(file, score =1000)
  open(file, "w") do |f|
    f.puts score, Time.new
  end
end

def load_game(file)
  score, time = IO.readlines(file)
  if cheating?(file, time)
    raise "This is cheating"
    print_score(score)
  end
end
...
def print_score(score)
  "Your saved score is #{score.to_i}"
end
